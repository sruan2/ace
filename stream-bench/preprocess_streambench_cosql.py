#!/usr/bin/env python3
"""
Download StreamBench (CoSQL subset) from Hugging Face and preprocess to:
(1) supplement database schema (db_schema) using CoSQL tables.json (preferred)
...
CoSQL is a conversational text-to-SQL dataset. Each dialogue contains multiple turns.
We extract individual question-SQL pairs from the conversations.

Examples:
  # Generate test split using CoSQL databases
  python preprocess_streambench_cosql.py \
    --cosql_root ./data/cosql \
    --tables_json ./data/cosql/tables.json \
    --split test \
    --out ./data/streambench_cosql_test.jsonl

  # Generate train split using CoSQL databases
  python preprocess_streambench_cosql.py \
    --cosql_root ./data/cosql \
    --tables_json ./data/cosql/tables.json \
    --split train \
    --out ./data/streambench_cosql_train.jsonl

  Note: First download CoSQL data:
...
def load_streambench_cosql(split: str):
    # Hugging Face datasets
    from datasets import load_dataset
    # Subset name is "cosql", splits include train/validation/test
    ds = load_dataset("appier-ai-research/StreamBench", "cosql", split=split)
    return ds

def read_tables_json(tables_json_path: str) -> Dict[str, Dict[str, Any]]:
    """
    Read Spider/CoSQL-style tables.json:
...
def find_sqlite_path(cosql_root: str, db_id: str) -> Optional[str]:
    """
    Try common CoSQL/Spider layouts. CoSQL typically has databases in a "database" subdirectory.
    """
    patterns = [
        os.path.join(cosql_root, "database", db_id, "*.sqlite"),
        os.path.join(cosql_root, "database", f"{db_id}.sqlite"),
        os.path.join(cosql_root, "database", db_id, f"{db_id}.sqlite"),
        os.path.join(cosql_root, "**", db_id, "*.sqlite"),
        os.path.join(cosql_root, "**", f"{db_id}.sqlite"),
    ]
    for pat in patterns:
...
def sqlite_introspect_schema(sqlite_path: str) -> Dict[str, Any]:
    ...
    # Store columns globally to emulate tables.json indices
...
def coerce_row(example: Dict[str, Any]) -> Dict[str, Any]:
    """
    StreamBench CoSQL fields: db_id, question, SQL, question_id, turn_id, difficulty
    CoSQL is conversational, so we have turn_id to track position in dialogue.
    """
    ...
    # Create unique identifier combining question_id and turn_id for multi-turn conversations
    unique_id = f"{qid}_turn{turn_id}" if turn_id is not None else str(qid)
...
def main():
    ...
    ap.add_argument("--out", default="streambench_cosql_test.jsonl")
    ap.add_argument("--tables_json", default="", help="Path to CoSQL-style tables.json (recommended).")
    ap.add_argument("--cosql_root", default="", help="Root dir where CoSQL sqlite databases live (fallback if tables.json missing).")
    ...
    # Load dataset
    ds = load_streambench_cosql(args.split)

    # Load schema map from tables.json if provided
    dbid_to_schema = {}
    if args.tables_json:
        if os.path.exists(args.tables_json):
            dbid_to_schema = read_tables_json(args.tables_json)
            print(f"Loaded schema metadata from {args.tables_json}")
        else:
            print(f"Warning: --tables_json not found at {args.tables_json}, will use SQLite introspection")
    else:
        print("No tables.json provided, will use SQLite introspection for schema")
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        for ex in ds:
            ...
            if schema_obj is None:
                if args.cosql_root:
                    sqlite_path = find_sqlite_path(args.cosql_root, db_id)
                    if sqlite_path:
    ...
    if n_missing_schema:
        print(f"WARNING: {n_missing_schema} rows had missing schema (schema_not_found). Provide --tables_json and/or --cosql_root.")
